...
float RelevanceProcessor::TF_IDF(const std::string &term, const std::string &document, const std::vector<std::string> &documents) {
    const float tf = TF(term, document);
    const float idf = IDF(term, documents);
    return tf * idf;
}

std::vector<float> RelevanceProcessor::stringToTFIDFVector(const std::string& document, const std::vector<std::string>& documents) {
