...
public:
    explicit RelevanceProcessor(std::string seekedInfo) :
        seekedInfo(std::move(seekedInfo)) {
    }
...
private:
    ...
    static float TF_IDF(const std::string &term, const std::string &document,
                        const std::vector<std::string> &documents);
    static std::vector<float> stringToTFIDFVector(const std::string &document,
                                                  const std::vector<std::string> &documents);
